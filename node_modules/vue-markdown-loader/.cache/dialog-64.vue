...
<el-dialog title="提示" v-model="dialogVisible" size="tiny" :before-close="handleClose">
  ...
  <span slot="footer" class="dialog-footer">
    ...
    <el-button type="primary" @click="dialogVisible = false">确 定</el-button>
                    ...
                    <p>需要设置<code>v-model</code>属性，它接收<code>Boolean</code>，当为<code>true</code>时显示 Dialog。Dialog 分为两个部分：<code>body</code>和<code>footer</code>，<code>footer</code>需要具名为<code>footer</code>的<code>slot</code>。<code>title</code>属性用于定义标题，它是可选的，默认值为空。本例通过显式改变<code>v-model</code>的值来打开 Dialog，此外我们还为 Dialog 实例提供了<code>open</code>和<code>close</code>方法，可以通过调用它们来打开/关闭 Dialog。最后，本例还展示了<code>beforeClose</code>的用法。</p>
...
<span class="hljs-tag">&lt;<span class="hljs-name">el-dialog</span> <span class="hljs-attr">title</span>=<span class="hljs-string">"收货地址"</span> <span class="hljs-attr">v-model</span>=<span class="hljs-string">"dialogTableVisible"</span>&gt;</span>
...
<span class="hljs-tag">&lt;<span class="hljs-name">el-dialog</span> <span class="hljs-attr">title</span>=<span class="hljs-string">"收货地址"</span> <span class="hljs-attr">v-model</span>=<span class="hljs-string">"dialogFormVisible"</span>&gt;</span>
...
<p>每个 <code>el-dialog</code> 实例都暴露了如下方法，用于在不显式改变 <code>v-model</code> 值的情况下打开 / 关闭实例：</p>
<table class="table"><thead>
<tr>
<th>方法名</th>
<th>说明</th>
